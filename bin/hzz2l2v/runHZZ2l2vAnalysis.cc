...
mon.addHistogram( new TH1F( "HT",     ";HT [GeV];Events", 50,0,500) );
  ...
  for(ev.toBegin(); !ev.atEnd(); ++ev){ iev++;
    ...
    if(photonTrigger && (isWGmc || isZGmc)){
      ...
      for(size_t ig=0; ig<gen.size(); ig++){
        if(gen[ig].status()!=3) continue; //Hugo FIXME: status 3 doesn't exist anymore!!!
    ...
    std::sort(extraLeptons.begin(), extraLeptons.end(), utils::sort_CandidatesByPt);
	 	...
	 	if (isMC_Wlnu_inclusive || isMC_Wlnu_HT100){ //Avoid double counting and make our W#rightarrow l#nu exclusif of the dataset with a cut on HT... So FIXME we have to update the json file to include the HT>100 in the normal plots
			bool isHT100 = false;
      	...
      	if(minDRlj<0.4 || minDRlg<0.4) continue;	
			...
			if(isMC_Wlnu_inclusive) mon.fillHisto("HT",    "hugo_isMC_Wlnu_inclusive_", vHT, weight);
			if(isMC_Wlnu_HT100) mon.fillHisto("HT",    "hugo_isMC_Wlnu_HT100_", vHT, weight);
    ...
    for(size_t ivar=0; ivar<nvarsToInclude; ivar++){
      ...
      for(unsigned int L=0;L<3;L++){  //Loop to assign a Z-->ll channel to photons
        ...
        else if(isMC_Zjets && gammaWgtHandler){
          if(selLeptons.size()==2  && gammaWgtHandler){  //this is not run if photon reweighting is activated to avoid mixing           
            for(size_t ilep=0; ilep<2; ilep++){
              dilId *= selLeptons[ilep].pdgId();
              int id(abs(selLeptons[ilep].pdgId()));
            ...
            evCat=eventCategoryInst.GetCategory(selJets,boson);            
    				...
    				if( abs(dilId)==121) photonWeightMain=gammaWgtHandler->getWeightFor(photonVars,string("ee")+evCat);
    				if( abs(dilId)==169) photonWeightMain=gammaWgtHandler->getWeightFor(photonVars,string("mumu")+evCat);

    				weight = weight * photonWeightMain;
