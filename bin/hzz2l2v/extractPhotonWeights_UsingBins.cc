...
int main(int argc, char* argv[]){
  ...
  std::vector<string> zDataDir;
	std::vector<string> DataDir;
  if(isData){
    //    gDataDir.push_back("Instr. MET");
    //gDataDir.push_back("#gamma data");
    //gDataDir.push_back("#gamma+jets");
    gDataDir.push_back("Z#rightarrow ee-#mu#mu_filt1113"); //le denominateur
    DataDir.push_back("Z#rightarrow ee-#mu#mu_filt1113"); //le denominateur
    zDataDir.push_back("data"); //le numerateur
    DataDir.push_back("data"); //le numerateur
  }else{
    gDataDir.push_back("#gamma+jets");
  ...
  std::vector<string> catL  = {"#mu#mu", "ee", "ll"};
  ...
  std::vector<string> var   = {"_qt", "_qmass", "_met", "_mt"};
	...
	std::vector<string> DataFile = {gDataFile, zDataFile};
	std::cout << __LINE__ << std::endl;
  ...
  for(unsigned int c=0;c<cat.size();c++){
  	for(unsigned int b=0;b<bin.size();b++){
  		for(unsigned int v=0;v<var.size();v++){
  			for(unsigned int da=0; da<DataFile.size(); da++){
	std::cout << __LINE__ << std::endl;

     		TFile* File = new TFile(DataFile[da].c_str(), "READ");
	...
	std::cout << __LINE__ << std::endl;
	std::cout << DataDir[d]+cat[c]+bin[b]+var[v] << std::endl;

        	if(DataHistos.find(DataDir[d]+cat[c]+bin[b]+var[v])==DataHistos.end()){
	std::cout << __LINE__ << std::endl;
	std::cout << DataDir[d]+cat[c]+bin[b]+var[v] << std::endl;
           	gROOT->cd(); //make sure that the file is saved in memory and not in file
           	DataHistos[DataDir[d]+cat[c]+bin[b]+var[v]] = (TH1D*)hist->Clone(); //create a new histo, since it's not found
        	}else{
	std::cout << __LINE__ << std::endl;
	std::cout << DataDir[d]+cat[c]+bin[b]+var[v] << std::endl;
           	DataHistos[DataDir[d]+cat[c]+bin[b]+var[v]]->Add(hist); //add to existing histogram
        	}
     		}
     		File->Close();
	std::cout << __LINE__ << std::endl;
  ...
  if(asym){
    ...
    for(unsigned int v=0;v<var.size();v++){
      ...
      for(unsigned int b=0;b<bin.size();b++){
        for(unsigned int c=0;c<cat.size();c++){
     			for(unsigned int d=0; d<DataDir.size(); d++){
     			if(DataHistos.find(DataDir[d]+cat[c]+bin[b]+var[v])==DataHistos.end()){continue;}
          TH1D* hist = DataHistos[DataDir[d]+cat[c]+bin[b]+var[v]];        
          hist = (TH1D*)hist->Clone((DataDir[d]+cat[c]+bin[b]+var[v]+"rebin").c_str());
          hist = (TH1D*)hist->Rebin((sizeof(xbins)/sizeof(double))-1, hist->GetName(), xbins);             
          hist->Scale(1.0, "width");
          DataHistos[DataDir[d]+cat[c]+bin[b]+var[v]+"rebin"] = hist;
       		}
       		}
      }
  ...
  for(unsigned int v=0;v<var.size();v++){
    ...
    for(unsigned int b=0;b<bin.size();b++){ 
    	for(unsigned int c=0; c<cat.size(); c++){
      if(DataHistos.find(DataDir[0]+cat[c]+bin[b]+var[v])==DataHistos.end()){printf("Histo missing for %s\n", (DataDir[0]+cat[c]+bin[b]+var[v]).c_str()); continue;}
      TH1D* histgamma = DataHistos[DataDir[0]+cat[c]+bin[b]+var[v]];

        if(DataHistos.find(DataDir[1]+cat[c]+bin[b]+var[v])==DataHistos.end()){printf("Histo missing for %s\n", (DataDir[1]+cat[c]+bin[b]+var[v]).c_str()); continue;}
        TH1D* hist = DataHistos[DataDir[1]+cat[c]+bin[b]+var[v]];
        hist = (TH1D*)hist->Clone((DataDir[1]+cat[c]+bin[b]+var[v]+"weight").c_str());
  ...
  for(unsigned int v=0;v<var.size();v++){
	...
	std::cout << __LINE__ << std::endl;
for(unsigned int d=0; d<DataDir.size(); d++){
        if(DataHistos.find(DataDir[d]+cat[c]+bin[b]+var[v])==DataHistos.end()){printf("Histo missing for %s\n", (DataDir[d]+cat[c]+bin[b]+var[v]).c_str()); continue;}
        TH1D* hist = DataHistos[DataDir[d]+cat[c]+bin[b]+var[v]];
        ...
        leg->AddEntry(hist, catL[c].c_str(), "LP"); 
	   		...
	   		c1->SaveAs((outDir + "/PhotonWeight"+DataDir[d]+cat[c]+bin[b]+var[v]+".png").c_str());   
} }
      leg->Draw("SAME");
    }
