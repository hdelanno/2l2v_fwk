...
*/
      ...
      std::sort(extraLeptons.begin(), extraLeptons.end(), utils::sort_CandidatesByPt);
			...
			//Adding the lepton requirements from JMAR talk
			//Exactly two leptons
			if (selLeptons.size() != 2 || extraLeptons.size() != 0) continue;
			//tight muons of opposite sign
			if ( fabs(selLeptons[0].pdgId()) != 13 || fabs(selLeptons[0].pdgId()) != 13 || selLeptons[0].pdgId()*selLeptons[1].pdgId() > 0) continue;
      //Not too close from the Z peak (why ?)
      if (fabs( (selLeptons[0].p4()+selLeptons[1].p4()).M() -91.0 ) >10 ) continue;
			//Cleaning jets from leptons dR(jet, muon) >0.4 (done below)
			
			//one of my two muons fire the HLT_IsoMu20* (not yet implemented)
      ...
      pat::JetCollection selJets;
        ...
        /* //This is the old version, let's use the newer version from JMAR
        ...
        */
        ...
        if(minDRlj<0.4) continue;
				...
				//New version from JMAR
				const reco::GenJet* ref = jet.genJet();
				TString jetType;
				if (ref){
					//if(reco::deltaR(jet, *ref) < 0.25) jetType = "truejetsid";
					//if(reco::deltaR(jet, *ref) > 0.5) jetType = "pujetsid";
					if(reco::deltaR(jet, *ref)) jetType = "truejetsid";
					else {jetType = "pujetsid";}
				}
				else {jetType = "pujetsid";}
      ...
      //////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        //cross-clean with selected leptons and photons
        float minDRlj(9999.);
        for(size_t ilep=0; ilep<selLeptons.size(); ilep++) minDRlj = TMath::Min( minDRlj, deltaR(jet,selLeptons[ilep]) );
        if(minDRlj<0.4) continue;
        ...
        if (associatedGenJet){
          //if(reco::deltaR(jet, *ref) < 0.25) jetType = "truejetsid";
          //if(reco::deltaR(jet, *ref) > 0.5) jetType = "pujetsid";
          if(reco::deltaR(jet, *associatedGenJet)) jetType = "truejetsid";
          else {jetType = "pujetsid";}
        }
        else {jetType = "pujetsid";}
        ...
        if (!passPFloose) continue;
				...
				//Here, we only look at true jets
				if(jetType == "pujetsid") continue;

				//On regarde au-dessus de 30GeV seulement
				if(pt <30) continue;
			...
			if(selJets.size() >0){
 			 	mon.fillProfile("profilePt_pt_jet_vs_pt_Z_above30_percent", tag, selJets[0].eta(), selJets[0].pt()/(selLeptons[0].p4()+selLeptons[1].p4()).pt(), weight);
 			 	mon.fillHisto("corr_j0_Z_pt", tag, selJets[0].pt(), (selLeptons[0].p4()+selLeptons[1].p4()).pt(), weight);
			}
/*
