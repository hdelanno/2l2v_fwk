...
if [[ $step -eq 1 ]]; then
   ...
   runAnalysisOverSamples.py -e runMVAStudy_v2 -j $CMSSW_BASE/src/UserCode/llvv_fwk/test/MVAStudy_v2/samples.json -o $CMSSW_BASE/src/UserCode/llvv_fwk/test/MVAStudy_v2/results -d  /store/group/phys_higgs/cmshzz2l2v/2014_03_20/ -c $CMSSW_BASE/src/UserCode/llvv_fwk/test/runAnalysis_cfg.py.templ -p "@useMVA=True @saveSummaryTree=True @runSystematics=True @automaticSwitch=False @is2011=False @jacknife=0 @jacks=0" -s $queue --report True $arguments
fi
...
if [[ $step -eq 3 ]]; then
   ...
   runPlotter --iEcm 13 --iLumi $INTLUMI --inDir $PWD/results/ --outDir $PWD/plots/ --outFile $PWD/plotter.root  --json $PWD/samples.json --no2D --noPlot --cutflow $arguments
fi
